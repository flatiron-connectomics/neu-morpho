...
def test_decode_parents_gives_adjacent_parents():
    """dijkstra3d's parental_field encoding is undocumented; pin it.

    It is a 1-based Fortran-order flat index (0 = root). If that ever changes,
    the un-invalidated-length selection would silently follow nonsense chains, so
    assert every decoded parent is 26-adjacent to its child.
    """
    import dijkstra3d
    import edt
    import kimimaro.skeletontricks as skt

    m = _tube(length=20, r=3)
    dbf = skt.zero2inf(np.asfortranarray(
        edt.edt(m, anisotropy=(1, 1, 1), black_border=False, order="F")))
    root = neutu_trace._find_root(m)
    field = dijkstra3d.parental_field(neutu_trace.neutu_pdrf(dbf), root)
    parent = neutu_trace.decode_parents(field)

    inside = np.flatnonzero(m.reshape(-1, order="F") != 0)
    have = inside[parent[inside] >= 0]
    assert len(have) > 10
    cz, cy, cx = neutu_trace._unflatten(have, m.shape)
    pz, py, px = neutu_trace._unflatten(parent[have], m.shape)
    step = np.maximum.reduce([np.abs(pz - cz), np.abs(py - cy), np.abs(px - cx)])
    assert step.max() <= 1, "decoded parent is not 26-adjacent to its child"


def test_uninvalidated_selection_matches_or_beats_daf_on_a_tube():
    """A clean tube has one branch; neither selector may invent more."""
    m = _tube(length=50, r=4, bend=True)
    daf = neutu_trace.skeletonize(m, select="daf")
    uninv = neutu_trace.skeletonize(m, select="uninvalidated")
    for s in (daf, uninv):
        e = np.asarray(s.edges)
        v = np.asarray(s.vertices)
        deg = np.bincount(e.ravel(), minlength=len(v)) if len(e) else np.zeros(len(v), int)
        assert int((deg == 1).sum()) <= 4
        assert len(v) > 0
